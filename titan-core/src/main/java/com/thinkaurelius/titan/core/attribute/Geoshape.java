...
/**
 * A generic representation of a geographic shape, which can either be a single
 * point, circle, box, or polygon. Use {@link #getType()} to determine the type
 * of shape of a particular Geoshape object. Use the static constructor methods
...
public class Geoshape {
	...
	private Geoshape(final float[][] coordinates) {
		...
		Preconditions.checkArgument(coordinates[0].length == coordinates[1].length
				&& coordinates[0].length > 0);
		for (int i = 0; i < coordinates[0].length; i++) {
			if (Float.isNaN(coordinates[0][i]))
				Preconditions.checkArgument(i == 1 && coordinates.length == 2
						&& coordinates[1][i] > 0);
			else
				Preconditions
						.checkArgument(isValidCoordinate(coordinates[0][i], coordinates[1][i]));
		}
	...
	/**
	 * Returns the number of points comprising this geoshape. A point and circle
	 * have only one point (center of cricle), a box has two points (the
	 * south-west and north-east corners) and a polygon has a variable number of
	 * points (>=3).
	 *
	 * @return
	 */
	...
	/**
	 * Returns the point at the given position. The position must be smaller
	 * than {@link #size()}.
	 *
	 * @param position
	 * @return
	...
	/**
	 * Returns the singleton point of this shape. Only applicable for point and
	 * circle shapes.
	 *
	 * @return
	 */
	...
	/**
	 * Returns the radius in kilometers of this circle. Only applicable to
	 * circle shapes.
	 * 
	 * @return
	 */
	...
	public boolean intersect(Geoshape other) {
		...
		return r == SpatialRelation.INTERSECTS || r == SpatialRelation.CONTAINS
				|| r == SpatialRelation.WITHIN;
	}
	...
	public Shape convert2Spatial4j() {
		...
		case BOX:
			return CTX.makeRectangle(getPoint(0).getSpatial4jPoint(), getPoint(1)
					.getSpatial4jPoint());
		case POLYGON:
	...
	public static final Geoshape point(final float latitude, final float longitude) {
		Preconditions.checkArgument(isValidCoordinate(latitude, longitude),
	 ...
	 * @return
	 */
	public static final Geoshape circle(final float latitude, final float longitude,
			final float radiusInKM) {
		Preconditions.checkArgument(isValidCoordinate(latitude, longitude),
				"Invalid coordinate provided");
		Preconditions.checkArgument(radiusInKM > 0, "Invalid radius provided [%s]", radiusInKM);
		return new Geoshape(new float[][] { new float[] { latitude, Float.NaN },
				new float[] { longitude, radiusInKM } });
	}
	...
	/**
	 * Constructs a new box shape which is identified by its south-west and
	 * north-east corner points
	 * 
	 ...
	 * @return
	 */
	public static final Geoshape box(final double southWestLatitude,
			final double southWestLongitude, final double northEastLatitude,
			final double northEastLongitude) {
		return box((float) southWestLatitude, (float) southWestLongitude,
				(float) northEastLatitude, (float) northEastLongitude);
	}
	...
	/**
	 * A single point representation. A point is identified by its coordinate on
	 * the earth sphere using the spherical system of latitudes and longitudes.
	 */
	...
	public static class GeoshapeSerializer implements AttributeSerializer<Geoshape> {
		...
		public Geoshape convert(Object value) {
			if (value.getClass().isArray()
					&& (value.getClass().getComponentType().isPrimitive() || Number.class
				...
				Preconditions.checkArgument(components != null,
						"Could not parse coordinates from string: %s", value);
				...
				try {
					...
					throw new IllegalArgumentException("Could not parse coordinates from string: "
							+ value, e);
				}
